...
**Actualización 2026-07-22 (cuarta pasada, v1.3.1 — corrección de consistencia):** un cuarto prompt (explícitamente de "corrección de consistencia") identificó y pidió resolver contradicciones internas dejadas por v1.3.0: (1) la desalineación A/B/C vs. `client_notification.schema.json` seguía "documentada" pero sin una fuente de verdad formal que la resolviera — se creó `DOCUMENTATION_SOURCE_OF_TRUTH.md` con la decisión explícita y `DOCUMENTOS/07_COMUNICACION_CON_EL_CLIENTE.docx` queda marcado HISTÓRICO (sin editar el binario, ver nota técnica en ese archivo); (2) los ejemplos JSON tenían campos `_comment*` que rompían la validación estricta (`additionalProperties: false`) — se limpiaron y la documentación se movió a `examples/client_notifications/README.md`; (3) `NormalizedEvent` duplicaba campos de `AssetContext`/`AuthEvidence`/`RiskResult` (mezcla real de capas, no solo un riesgo teórico) — se corrigió; (4) el `case_type=VULNERABILITY_KEV` cambiaba de identidad al escalar un CVE genérico a KEV, violando la estabilidad de fingerprint — se corrigió a `case_type=VULNERABILITY` único; (5) los ejemplos afirmaban `approved_by: "EJEMPLO-NO-REAL"` con `status: approved`, simulando una aprobación humana inexistente — corregido a `status: draft`; (6) se detectaron afirmaciones de capacidad inexistente en el texto de los correos de ejemplo ("estamos revisando qué hizo la cuenta", "podemos bloquear la dirección") — corregidas a lenguaje honesto sobre el estado actual del repositorio; (7) el modelo de activo no admitía un tenant residencial (Casa del CEO) — se separó `environment`/`site_type`. Ver v1.3.1 en `REGLAS_TUNEADAS/CHANGELOG.md` para el detalle exhaustivo de cada corrección, y las secciones de entrega de esa misma pasada para los resultados de validación.

**Actualización 2026-07-26 (quinta pasada, v1.3.1 — Architectural Consistency):** un quinto prompt, esta vez enmarcado como auditoría integral de arquitectura ("arquitecto principal de software, seguridad y plataforma SOC"), pidió revisar el working tree completo (que ya contenía trabajo local sin confirmar de una v1.3.1 previa — reconciliado, no sobrescrito) y ejecutar 28 fases de corrección. Resultado: 1 hallazgo P0 (`rule_exclude`/whitelist FIM tratados como personalización por tenant en un Manager compartido — corregido con explicación completa de alternativas válidas), 8 documentos nuevos en `docs/architecture/`, el primer ADR del repositorio, una política de riesgo determinística (`policies/risk/risk_policy_v1.yaml`), un contrato de mensajería entrante (`specifications/inbound_message.schema.json`), un sexto procedimiento (`PROC-VULN-KEV-MC-001`), y 9 checks nuevos de validación automática — incluyendo la corrección de 2 bugs reales en el propio tooling de CI (comparación de fin de línea que hacía fallar siempre `generate_routing_matrix.py --check`, y un `TypeError` en el nuevo check de máquina de estados única). Ver v1.3.1 — Architectural Consistency en `REGLAS_TUNEADAS/CHANGELOG.md` para el detalle exhaustivo, y el reporte de entrega de esa misma pasada para la clasificación completa P0/P1/P2 y el estado final.
