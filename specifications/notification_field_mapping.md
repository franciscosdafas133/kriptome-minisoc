...
| Tipo de entorno | `environment_type` | `asset_context` (v1.3.1, ya no vive en `incident_facts` -- ver separación de capas en `normalized_event.schema.json`) | Label `agent.labels.environment_type`, o registro manual de onboarding para tenants residenciales (valor `personal`) | Opcional | Omitir | No (dato técnico interno, va a `technical_appendix` si se necesita) | interna |
| Tipo de sitio | `site_type` | `asset_context` | Registro manual de onboarding (p. ej. `home` para un tenant residencial, `office` para una empresa) | Opcional | Omitir | No | interna |
