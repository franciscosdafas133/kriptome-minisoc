...
5. **`policy_scope_status: DESCARTADA-PARA-PRODUCCION`** (usado en `policies/case_types/VULNERABILITY_KEV.yaml` y en `REGLAS_TUNEADAS/experimental/100040_cisa_kev.xml`) es una anotación de texto libre, no un 11º estado de este vocabulario cerrado. Se usa exclusivamente para marcar un artefacto que YA tuvo una decisión de arquitectura resuelta en su contra (a diferencia de `EXPERIMENTAL`, que implica la decisión aún pendiente) — el artefacto conserva su `validation_status` técnico normal (p. ej. sigue siendo `VALIDADO-XML` en su sintaxis) y además lleva esta nota de alcance.
